(function () {
    ...
    angular.module('tpjApp')
            .controller('LaporanRekapMerkController', LaporanRekapMerkController)
    function LaporanRekapMerkController($scope, KotaService, StuffingService, KapalBerangkatService) {
        $scope.kota = {};
        $scope.kapal = {};
        $scope.open = open;
        $scope.opened = false;
        ...
        $scope.cetak = function (c, tipe) {
            var link = 'api/report/rekap-merk.' + tipe + '?id=' + c.id;
            if (tipe == 'pdf') {
